...
open class AudioFloatConversion32L : AudioFloatConverter() {
    ...
    // Convert 4 bytes to a float in little-endian order
    private fun bytesToFloat(bytes: ByteArray, index: Int): Float {
        val intBits = (bytes[index].toInt() and 0xff) or
    ...
    // Convert a float to 4 bytes in little-endian order
    private fun floatToBytes(value: Float, bytes: ByteArray, index: Int) {
        val intBits = value.toBits()
        bytes[index] = (intBits and 0xff).toByte()
        bytes[index + 1] = ((intBits shr 8) and 0xff).toByte()
        bytes[index + 2] = ((intBits shr 16) and 0xff).toByte()
        bytes[index + 3] = ((intBits shr 24) and 0xff).toByte()
    }
}